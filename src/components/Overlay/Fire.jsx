...
export default function Fire() {
  ...
  const { position, size } = useMemo(
    function factory() {
      const width = TEXTURES[OVERLAY.FIRE].width * screen.unit;
      const height = TEXTURES[OVERLAY.FIRE].width * screen.unit;
      return {
        position: [
  ...
  function onDown(event) {
    down({ event, key: SETTINGS.KEYS.FIRE, keyCode: SETTINGS.KEYS.FIRE });
    shader.current.uniforms.u_offset.value[1] = 0;
  }

  function onUp(event) {
    up({ event, key: SETTINGS.KEYS.FIRE, keyCode: SETTINGS.KEYS.FIRE });
